...
```{r}
# Histogram of log(outcome) and residual plot of LMM
library(car)
library(dplyr)

OUTPATH="/home/kenneth/out/metabolic/CPH.20211217/paper20231025/figures/"

png(paste0(OUTPATH, "outcome_qqplot.png"), width=480, height=680)
op <- par(mfrow = c(4, 3), mar=c(4, 4, 6, 1.5))
qqnorm(df.bt.final$cholestrol, main="TC")
qqline(df.bt.final$cholestrol)
qqnorm(df.bt.final$HDL_Chol, main="HDL")
qqline(df.bt.final$HDL_Chol)
qqnorm(df.bt.final$LDL_Chol, main="LDL")
qqline(df.bt.final$LDL_Chol)
qqnorm(log(df.bt.final$cholestrol), main="log(TC)")
qqline(log(df.bt.final$cholestrol))
qqnorm(log(df.bt.final$HDL_Chol), main="log(HDL)")
qqline(log(df.bt.final$HDL_Chol))
qqnorm(log(df.bt.final$LDL_Chol), main="log(LDL)")
qqline(log(df.bt.final$LDL_Chol))

qqnorm(df.bt.final$Triglycerides, main="TG")
qqline(df.bt.final$Triglycerides)
qqnorm(df.bg.final$BGF, main="BG")
qqline(df.bg.final$BGF)
qqnorm(df.bc.final$BMI, main="BMI")
qqline(df.bc.final$BMI)
qqnorm(log(df.bt.final$Triglycerides), main="log(TG)")
qqline(log(df.bt.final$Triglycerides))
qqnorm(log(df.bg.final$BGF), main="log(BG)")
qqline(log(df.bg.final$BGF))
qqnorm(log(df.bc.final$BMI), main="log(BMI)")
qqline(log(df.bc.final$BMI))
main_title <- "Normal quantile plot for metabolic measures before and after log transformation"
mtext(main_title, side=3, outer = T, line=-2, font = 0.8, cex = 1.0)
...
# QQ-Plot of Residuals in LMM models
#m <- models[[m_type[1]]]
m <- models[[m_type[2]]]
#png(paste0(OUTPATH, "residual_qqplot_modelA.png"), width=960, height=640)
png(paste0(OUTPATH, "residual_qqplot_modelB.png"), width=960, height=640)
op <- par(mfrow = c(2, 3), mar=c(4, 4, 7, 1.5))
for(i in 1:length(m)){
  r <- scale(residuals(m[[i]]))
  qqnorm(r, main=m_names[i], ylab="Standardized residuals")
  qqline(r)
}
#main_title <- "Normal quantile plot for standardized residual (Model A)"
main_title <- "Normal quantile plot for standardized residual (Model B)"
mtext(main_title, side=3, outer = T, line=-2, font = 0.8, cex = 1.2)
par(op)
dev.off()

# Residual vs fitted value plots of LMM model
#m <- models[[m_type[1]]]
m <- models[[m_type[2]]]
#model_name <- "Model A"
model_name <- "Model B"
...
for(i in 1:length(m)){
  lmm <- m[[i]]
  #scatterplot(sqrt(abs(residuals(lmm))) ~ fitted(lmm),
  scatterplot(residuals(lmm) ~ fitted(lmm),
            xlab=paste0("Estimated ", m_names[i]) , 
            ylab=paste0("Residual of ", model_name, " (", m_names[i], ")"))
}
...
# Between-subject (Model A), remove the subject with education = NA
df.tmp <- subset(df.bc.final[!is.na(df.tmp$education),], select=c(age, Gender, education, tx_duration, bz_CLOZAPINE, bz_OLANZAPINE, bz_ARIPIPRAZOLE, bz_AMISULPRIDE, bz_QUETIAPINE, bz_RISPERIDONE, bz_PALIPERIDONE, bz_METFORMIN, bz_ATORVASTATIN, bz_SIMVASTATIN, bz_VALPROATE, bz_LITHIUM, bz_CITALOPRAM))

cor_matrix <- cor(df.tmp)
p.mat <- corr.test(df.tmp)$p
ggcorrplot(cor_matrix, method="square", type="upper", p.mat=p.mat, tl.cex=8, tl.srt=45,
           title="Correlation plot for between-subject covariates (Model A)")
ggsave(file=paste0(OUTPATH, "corrplot_bz_modelA.png"), device="png")

# Within-subject (Model A), remove the subject with education = NA
df.tmp <- subset(df.bc.final[!is.na(df.tmp$education),], select=c(age, Gender, education, tx_duration, wz_CLOZAPINE, wz_OLANZAPINE, wz_ARIPIPRAZOLE, wz_AMISULPRIDE, wz_QUETIAPINE, wz_RISPERIDONE, wz_PALIPERIDONE, wz_METFORMIN, wz_ATORVASTATIN, wz_SIMVASTATIN, wz_VALPROATE, wz_LITHIUM, wz_CITALOPRAM))


cor_matrix <- cor(df.tmp)
p.mat <- corr.test(df.tmp)$p
ggcorrplot(cor_matrix, method="square", type="upper", p.mat=p.mat, tl.cex=8, tl.srt=45,
           title="Correlation plot for within-subject covariates (Model A)")
ggsave(file=paste0(OUTPATH, "corrplot_wz_modelA.png"), device="png")


# Between-subject (Model B), remove the subject with education = NA
df.tmp <- subset(df.bc.final[!is.na(df.tmp$education),], select=c(age, Gender, education, tx_duration, bz_dose.CLOZAPINE, bz_dose.OLANZAPINE, bz_dose.ARIPIPRAZOLE, bz_dose.AMISULPRIDE, bz_dose.QUETIAPINE, bz_dose.RISPERIDONE, bz_dose.PALIPERIDONE, bz_dose.METFORMIN, bz_dose.ATORVASTATIN, bz_dose.SIMVASTATIN, bz_dose.VALPROATE, bz_dose.LITHIUM, bz_dose.CITALOPRAM))

cor_matrix <- cor(df.tmp)
p.mat <- corr.test(df.tmp)$p
ggcorrplot(cor_matrix, method="square", type="upper", p.mat=p.mat, tl.cex=8, tl.srt=45,
           title="Correlation plot for between-subject covariates (Model B) ")
ggsave(file=paste0(OUTPATH, "corrplot_bz_modelB.png"), device="png")

# Within-subject (Model B), remove the subject with education = NA
df.tmp <- subset(df.bc.final[!is.na(df.tmp$education),], select=c(age, Gender, education, tx_duration, wz_dose.CLOZAPINE, wz_dose.OLANZAPINE, wz_dose.ARIPIPRAZOLE, wz_dose.AMISULPRIDE, wz_dose.QUETIAPINE, wz_dose.RISPERIDONE, wz_dose.PALIPERIDONE, wz_dose.METFORMIN, wz_dose.ATORVASTATIN, wz_dose.SIMVASTATIN, wz_dose.VALPROATE, wz_dose.LITHIUM, wz_dose.CITALOPRAM))

cor_matrix <- cor(df.tmp)
p.mat <- corr.test(df.tmp)$p
ggcorrplot(cor_matrix, method="square", type="upper", p.mat=p.mat, tl.cex=8, tl.srt=45,
           title="Correlation plot for within-subject covariates (Model B) ")
ggsave(file=paste0(OUTPATH, "corrplot_wz_modelB.png"), device="png")


```

```{r include=TRUE, warning=TRUE, message=TRUE}
### Building Linear Mixed Model for stratified analysis of SGA drugs
...
# Generate the summary statistic of the model coefficients
axis_label <- c("Take 1st Gen Drug(FGA)", "Tx duration(mths)", "Take 2nd Gen Drug(SGA)", "Education", "Sex(Female)", "Age")
...
## OLANZAPINE (Threshold = 5mg)
t <- 'TC'
drug <- 'OLANZAPINE'
df.startup <- df.bt.final.tmp[df.bt.final.tmp$dose.OLANZAPINE <= 5.0,]
df.ongoing <- df.bt.final.tmp[df.bt.final.tmp$dose.OLANZAPINE >= 5.0,]

# LMM model for startup (0 - 5mg)
TC_mixed_model_dose.startup <- lmer(log_cholestrol~age+Gender+education+tx_duration+dose.HALOPERIDOL+dose.TRIFLUOPERAZINE+dose.CLOZAPINE+dose.OLANZAPINE+dose.ARIPIPRAZOLE+dose.AMISULPRIDE+dose.QUETIAPINE+dose.RISPERIDONE+dose.PALIPERIDONE+dose.METFORMIN+dose.ATORVASTATIN+dose.SIMVASTATIN+dose.VALPROATE+dose.LITHIUM+dose.CITALOPRAM+dose.SERTRALINE
                         +(1+dose.OLANZAPINE|ID), df.startup)
#summary(TC_mixed_model_dose.startup)

# LMM model for ongoing (>5mg)
TC_mixed_model_dose.ongoing <- lmer(log_cholestrol~age+Gender+education+tx_duration+dose.HALOPERIDOL+dose.TRIFLUOPERAZINE+dose.CLOZAPINE+dose.OLANZAPINE+dose.ARIPIPRAZOLE+dose.AMISULPRIDE+dose.QUETIAPINE+dose.RISPERIDONE+dose.PALIPERIDONE+dose.METFORMIN+dose.ATORVASTATIN+dose.SIMVASTATIN+dose.VALPROATE+dose.LITHIUM+dose.CITALOPRAM+dose.SERTRALINE
                         +(1+dose.OLANZAPINE|ID), df.ongoing)
#summary(TC_mixed_model_dose.ongoing)

models <- list(TC_mixed_model_dose.startup, TC_mixed_model_dose.ongoing)
title <- paste0('Percentage change of ', t, ' per mg of ', drug, ' taken')
model_names <- c('0 to 5mg', '5mg to 30mg')
visualize_model(models, model_names, axis_label, title, "exp2percentInc")



## METFORMIN (Threshold = 250mg)
t <- 'TC'
drug <- 'METFORMIN'
df.startup <- df.bt.final.tmp[df.bt.final.tmp$dose.METFORMIN <= 1500,]
df.ongoing <- df.bt.final.tmp[df.bt.final.tmp$dose.METFORMIN >= 1500,]

# LMM model for startup (0 - 1500mg)
TC_mixed_model_dose.startup <- lmer(log_cholestrol~age+Gender+education+tx_duration+dose.HALOPERIDOL+dose.TRIFLUOPERAZINE+dose.CLOZAPINE+dose.OLANZAPINE+dose.ARIPIPRAZOLE+dose.AMISULPRIDE+dose.QUETIAPINE+dose.RISPERIDONE+dose.PALIPERIDONE+dose.METFORMIN+dose.ATORVASTATIN+dose.SIMVASTATIN+dose.VALPROATE+dose.LITHIUM+dose.CITALOPRAM+dose.SERTRALINE
                         +(1+dose.METFORMIN|ID), df.startup)
#summary(TC_mixed_model_dose.startup)

# LMM model for ongoing (> 1500mg)
TC_mixed_model_dose.ongoing <- lmer(log_cholestrol~age+Gender+education+tx_duration+dose.HALOPERIDOL+dose.TRIFLUOPERAZINE+dose.CLOZAPINE+dose.OLANZAPINE+dose.ARIPIPRAZOLE+dose.AMISULPRIDE+dose.QUETIAPINE+dose.RISPERIDONE+dose.PALIPERIDONE+dose.METFORMIN+dose.ATORVASTATIN+dose.SIMVASTATIN+dose.VALPROATE+dose.LITHIUM+dose.CITALOPRAM+dose.SERTRALINE
                         +(1+dose.METFORMIN|ID), df.ongoing)

#summary(TC_mixed_model_dose.ongoing)
models <- list(TC_mixed_model_dose.startup, TC_mixed_model_dose.ongoing)
title <- paste0('Percentage change of ', t, ' per mg of ', drug, ' taken')
model_names <- c('0 to 1500mg', '1500mg to 3000mg')
visualize_model(models, model_names, axis_label, title, "exp2percentInc")
